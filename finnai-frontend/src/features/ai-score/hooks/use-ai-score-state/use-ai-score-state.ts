...
import { isScorePopulated, hasDisplayableScore } from "../../utils/optimistic-score";
...
export function useAIScoreState() {
  ...
  const status: AIScoreUiStatus = useMemo(() => {
    ...
    if (score?.status === "failed" && !hasDisplayableScore(score)) return "error";
    if (!score && !query.isLoading) return "empty";
    if (score && !isPopulated && !hasDisplayableScore(score)) return "generating";
    return "ready";
  }, [
    query.isLoading,
    query.isError,
  ...
  const regenerationFailed =
    score?.status === "failed" && hasDisplayableScore(score) && !isGenerating;
  const history = score && (isPopulated || hasDisplayableScore(score)) ? getHistory(score.workspace_id) : [];
